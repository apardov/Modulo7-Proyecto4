...
def order_details(request, pedido_id):
    try:
        order_detail = DetallePedido.objects.get(id=pedido_id)
...
def order_status(request, pedido_id):
    order_details = get_object_or_404(Pedido, id=pedido_id)
...
def take_order(request):
    if request.method == 'POST':
        ...
        if take_order_form.is_valid():
            take_order_form.save()
            messages.success(request, "Pedido generado con exito")
            return redirect('gestion:authenticated')
    else:
        take_order_form = TakeOrderForm()
        return render(request, template_name='gestion/order.html',
                      context={'take_order_form': take_order_form})
